// ============================================================================
// Playgama Bridge v2 backend cho SdkHandler (multi-backend, M2 Phase 5).
// Đóng gói window.bridge / window.playgamaBridge với cùng interface SdkHandler,
// để scenes (Gameplay/Start/main.ts/context.ts) gọi KHÔNG đổi.
// Mirrors M1/M3 (M1-Rescue-Dodge/game/src/sdk-bridge-backend.ts).
//
// QUAN TRỌNG (bug real gặp 2026-08-24): bridge THROW ("Before using the SDK you
// must initialize it") nếu gọi API trước khi bridge.initialize() resolve. Vì
// main.ts gọi sdk.isAudioEnabled/onPause/onAudioEnabledChange/gameReady NGAY lúc
// boot → backend PHẢI buffer mọi subscribe tới khi ready, getter trả default an
// toàn, gameReady/storage/ads no-op hoặc defer. KHÔNG truy cập bridge.platform
// /advertisement trước ready.
//
...
/** Phát hiện Playgama Bridge trên window (host portal inject). */
export function getBridge(): PlaygamaBridgeLike | null {
  if (typeof window === 'undefined') return null;
  return window.bridge ?? window.playgamaBridge ?? null;
}

/** Key lưu tiến trình cho bridge.storage (cloud-save khi nền tảng hỗ trợ). */
export const LOCAL_KEY = 'neon_sort_save';
...
export class PlaygamaBackend {
  ...
  private async _init(): Promise<void> {
    try {
      ...
      console.warn('[sdk] playgama bridge.initialize failed (mock/unsupported?)', e);
    }
  ...
  /** Playgama chỉ có 1 tin readiness: game_ready (first_frame không tồn tại). */
  gameReady(): void {
    if (!this.ready) {
  ...
  /** firstFrameReady không có API riêng trên Playgama → no-op (game_ready là đủ). */
  firstFrameReady(): void { /* no-op */ }
  ...
  /** Ngôn ngữ hoạt động từ platform (Playgama locale, ISO 639-1; default en). */
  getLanguage(): string {
    if (!this.ready) return 'en';
    try {
      const l = this.bridge.platform.language;
  ...
  async saveData(data: unknown): Promise<boolean> {
    ...
    try {
      ...
      console.warn('[sdk] bridge.storage.set failed', e);
      return false;
    }
  }
  ...
  async loadData(): Promise<unknown | null> {
    ...
    try {
      ...
      console.warn('[sdk] bridge.storage.get failed', e);
      return null;
    }
  }
  ...
  isRewardedAvailable(): boolean {
    if (!this.ready) return true;             // chưa ready → lạc quan; request tự guard
    try { return this.bridge.advertisement.isRewardedSupported !== false; }
    catch { return true; }
  }

  isInterstitialAvailable(): boolean {
    if (!this.ready) return false;
    try { return this.bridge.advertisement.isInterstitialSupported !== false; }
    catch { return false; }
  }
