...
7. **M5–M8 — boss verdict 08/09: chơi CHÁN, xoá làm lại** (lệnh boss, không phải gate kỹ thuật): 4 game chain dài (nhiều thẻ, nhiều vòng QA) ra sản phẩm đủ gate nhưng nhạt; đối chứng M3/M4 làm bằng **5 file spec chốt trước + 1 prompt chạy trọn = MVP đạt**. Hành động: `git rm` M5–M8 (lịch sử vẫn trong git + bản đã push origin), luật mới **§PB-2b SPEC-DRIVEN** trong PROJECT-RULES: PHASE 0 spec đủ 5 file là gate cứng trước mọi card code; coding agent chạy trọn PROMPT một mạch ra MVP, verify bằng số rồi mới trình.
