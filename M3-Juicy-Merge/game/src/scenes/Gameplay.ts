...
import {
  computeBucketLayout,
  type BucketLayout,
} from "../gameplay/physics-layout";
import {
  resolveFruitTexture,
  fruitRadius,
  fruitDiameter,
} from "../gameplay/fruit-sprite";
import { type CollidingFruit, type MergePlan } from "../gameplay/merge-handler";
import {
  playJuiceSplash,
  playJackpotClimax,
  playFireworksCelebration,
  computeComboDetune,
...
export class GameplayScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.dangerCountdownText = this.add
      .text(width / 2, this.layout.dangerY - 32, "", {
    ...
    this.ghostX = Phaser.Math.Clamp(
      width / 2,
      this.layout.bucketX0 + fruitRadius(this.ghostTier),
      this.layout.bucketX1 - fruitRadius(this.ghostTier),
    );
    this.refreshGhost();
  ...
  private drawBucket(): void {
    ...
    g.fillRoundedRect(
      ...
      L.bucketWidth + 20,
      H + 16,
      { tl: 0, tr: 0, bl: radius.lg, br: radius.lg },
    );
    ...
    g.strokeRoundedRect(
      ...
      L.bucketWidth - 2,
      H - 2,
      { tl: 0, tr: 0, bl: radius.md, br: radius.md },
    );
  ...
  override update(time: number, _delta: number): void {
    ...
    const isAboveDanger = (f: DroppedFruit): boolean => {
      ...
      return (
        f.obj.y < this.layout.dangerY || f.obj.y - r * 0.4 < this.layout.dangerY
      );
    };
    ...
    const nearDanger =
      hasDangerFruit ||
      this.fruits.some((f) => f.obj.y < this.layout.dangerY + DANGER_NEAR_BAND);
    ...
    if (hasDangerFruit) {
      ...
      this.dangerCountdownText
        .setText(`⚠️ DANGER: ${remainingSec}s`)
        .setAlpha(1);
  ...
  clearFruitsAboveDanger(): void {
    ...
    for (const f of this.fruits) {
      const r = fruitRadius(f.tier);
      if (
        f.obj.y < this.layout.dangerY ||
        f.obj.y - r * 0.5 < this.layout.dangerY
      ) {
        toRemoveSet.add(f);
      }
    }
  ...
  private createHud(): void {
    ...
    const swapContainer = this.add
      .container(this.swapButtonBaseX, 16)
      .setDepth(z.hud);
    ...
    const shakeW = 104;
    const shakeH = 72;
    const shakeContainer = this.add
      .container(this.shakeButtonBaseX, 16)
      .setDepth(z.hud);
    ...
    if (ctx.isDailyMode) {
      ...
      this.dailyBannerText = this.add
        .text(
          width / 2,
          bannerY + bannerH / 2,
          `📅 Day ${diff.dayLevel}/12: ${diff.fruitLimit} fruits left   •   Goal: ${diff.targetScore} pts 🎯`,
          {
            fontFamily: "sans-serif",
            fontSize: "15px",
            fontStyle: "bold",
  ...
  private updateHud(): void {
    ...
    this.bestScoreText.setText(
      `🏆 BEST ${Math.max(ctx.engine.state.bestScore, ctx.engine.state.score)}`,
    );
    if (this.swapCountText) {
      this.swapCountText.setText(`x${ctx.engine.powerups.swapCount} Swap`);
      this.swapCountText.setColor(
        ctx.engine.powerups.swapCount > 0 ? "#10B981" : "#9CA3AF",
      );
    }
    if (this.shakeCountText) {
      this.shakeCountText.setText(`x${ctx.engine.powerups.shakeCount}`);
      this.shakeCountText.setColor(
        ctx.engine.powerups.shakeCount > 0 ? "#8B5CF6" : "#9CA3AF",
      );
    }
    if (this.dailyBannerText && ctx.isDailyMode) {
      ...
      if (reached) {
        this.dailyBannerText.setText(
          `🎉 DAY ${diff.dayLevel} COMPLETED: ${ctx.engine.state.score}/${diff.targetScore} pts (Left ${remaining}) 🏆`,
        );
        this.dailyBannerText.setColor("#059669");
      } else {
        this.dailyBannerText.setText(
          `📅 Day ${diff.dayLevel}/12: ${remaining}/${diff.fruitLimit} fruits left   •   Goal: ${diff.targetScore} pts 🎯`,
        );
  ...
  private promptRefillPowerups(): void {
    ...
    const btnW = cardW - 64;
    const { container: watchBtn } = drawButton(
      this,
      0,
      50,
      "▶  WATCH AD (+2 🔄 & +2 📳)",
  ...
  private showCosmicVictoryModal(): void {
    ...
    const subtitle = this.add
      .text(
        0,
        -cardH / 2 + 120,
        "You created the Ultimate Tier 14\nGalaxy Watermelon! 🌌",
        {
          fontFamily: "sans-serif",
          fontSize: "15px",
          fontStyle: "bold",
    ...
    // Buttons
    const btnW = cardW - 70;
    const { container: keepPlayingBtn } = drawButton(
      this,
      0,
      175,
      "▶  KEEP PLAYING",
  ...
  private flashCombo(): void {
    ...
    const colHex =
      comboColors[Math.min(n - 2, comboColors.length - 1)] ?? "#FFC048";
  ...
  private createGhost(): void {
    ...
    this.ghost = this.add
      .image(this.ghostX, this.layout.spawnY, key)
      .setDisplaySize(d, d)
      .setAlpha(0.88)
      .setDepth(z.hud);
      ...
      this.time.delayedCall(2000, () => {
        if (!this.gameOverTriggered) {
          ...
          if (
            ...
            !this.hasClaimedDailyExtraDrops
          ) {
  ...
  private processPendingMerges(): void {
    ...
    if (plans.length > 0) {
      ...
      const popX = lastPlan
        ? (this.fruitsById.get(lastPlan.aId)?.obj.x ?? this.scale.width / 2)
        : this.scale.width / 2;
...
function fruitOf(
  body: MatterBodyHandle | null | undefined,
): CollidingFruit | null {
  if (!body) return null;
  ...
  const go = (anyBody.gameObject ??
    anyBody.parent?.gameObject) as Phaser.Physics.Matter.Image | null;
