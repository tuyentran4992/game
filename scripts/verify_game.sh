...
# Rào ① type-check + test logic · Rào ② build + boot-asset check
...
echo -e "\n${HASH_FMT}[1/4] Type check (tsc --noEmit)${NC}"
...
echo -e "\n${HASH_FMT}[2/4] Logic tests (vitest run)${NC}"
...
# ---------- Rào ②a : BUILD ----------
echo -e "\n${HASH_FMT}[3/4] Build (npm run build)${NC}"
...
echo -e "\n${HASH_FMT}[4/4] Asset manifest check (code load keys vs raw files)${NC}"
...
echo -e "\n${GREEN}═════ VERIFY $GAME PASSED (type + logic + build; asset cảnh báo riêng) ═════${NC}"