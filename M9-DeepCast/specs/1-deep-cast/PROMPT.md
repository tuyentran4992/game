...
> Cách dùng (như M3): `cd /data/youtube-playables/M9-DeepCast && claude -p "$(cat specs/1-deep-cast/PROMPT.md)" --model opus --max-turns 60` (background, nguồn qua AI-Box).
...
3. Art: bộ SVG production theo DESIGN-SPEC §3 → script build ra PNG atlas vào `public/assets` (<4MB, TB-03).
