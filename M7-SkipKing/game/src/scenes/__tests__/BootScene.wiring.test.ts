// BUG-L2-01 TDD regression — BootScene.create() PHẢI handoff scene.start('OnboardingPlayScene').
...
// mãi, OnboardingPlayScene:idle mãi. Test này boot Phaser thật (headless jsdom — mô hình T1e M1),
// đăng ký OnboardingPlayScene STUB (autoStart=false — CHỈ active khi BootScene handoff), rồi assert
// OnboardingPlayScene trở nên ACTIVE TỰ ĐỘNG sau preload.
...
// ScenePlugin.prototype. Trong test này CHỈ BootScene có thể gọi start('OnboardingPlayScene')
// (OnboardingPlayScene là stub trơ, autoStart=false). Restore ở afterAll.
...
beforeAll(async () => {
  ...
  game.scene.add('OnboardingPlayScene', new Phaser.Scene('OnboardingPlayScene'), false);
  await waitFor(
    () => !!game && game.scene.isActive('OnboardingPlayScene'),
    'OnboardingPlayScene không bao giờ ACTIVE — BootScene không handoff scene.start (BUG-L2-01 tái hiện: màn đen)',
  );
}, 20000);

afterAll(() => { game?.destroy(true); vi.restoreAllMocks(); });

describe('BootScene handoff — create() chuyển OnboardingPlayScene (BUG-L2-01 regression)', () => {
  ...
  it('create() gọi scene.start("OnboardingPlayScene") — đúng 1 lệnh handoff', () => {
    const playStarts = startSpy.mock.calls.filter((args) => args[0] === 'OnboardingPlayScene');
    expect(playStarts.length).toBe(1);
  });

  it('OnboardingPlayScene ACTIVE tự động sau preload + BootScene dừng sau khi handoff', () => {
    expect(game!.scene.isActive('OnboardingPlayScene')).toBe(true);
